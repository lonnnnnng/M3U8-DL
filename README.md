...
- CENC/SAMPLE-AES/SAMPLE-AES-CTR 下载后外部工具解密入口，含基础实时分片解密路径；实时 MP4 解密会先保留原始 init 读取 KID，本地读不到且使用 shaka 时会先从缺 key 错误探测 `key_id`，再解密 init 和后续媒体分片，并避免合并后重复整文件解密
