...
- CENC/SAMPLE-AES/SAMPLE-AES-CTR 下载后外部工具解密入口，含基础实时分片解密路径；实时 MP4 解密会先保留原始 init 读取 KID，再解密 init 和后续媒体分片，并避免合并后重复整文件解密
...
- 最终 ffmpeg 混流清理输入 metadata、复制未知流，并设置默认视频/首音频/字幕非默认轨道标记
