...
func (p *parser) cloneForLiveTrack() *parser {
	if p == nil {
		return nil
	}
	clone := *p
	clone.rawFiles = make(map[string]string, len(p.rawFiles))
	for name, raw := range p.rawFiles {
		clone.rawFiles[name] = raw
	}
	// long: 每条直播轨道会并发切换解析基准 URL；独立 parser 状态可避免慢轨改写其他轨道的相对地址解析结果。
	return &clone
}
