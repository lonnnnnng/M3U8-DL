...
BRANDED_APP_PATH="${DESKTOP_DIR}/build/bin/M3U8-DL.app"
if [[ "${APP_PATH}" != "${BRANDED_APP_PATH}" ]]; then
  # long: 保留内部 Wails 项目标识以维持 Bundle ID 稳定，只在打包阶段改成用户可见的产品名称。
  mv "${APP_PATH}" "${BRANDED_APP_PATH}"
  APP_PATH="${BRANDED_APP_PATH}"
fi
...
LEGACY_ARCHIVE="${DIST_DIR}/M3U8-DL_desktop_macos_${ARCH_SUFFIX}.zip"
PREVIOUS_BRAND_ARCHIVE="${DIST_DIR}/m3u8dl-go_${RELEASE_VERSION}_desktop_macos_${ARCH_SUFFIX}.zip"
# 旧版无版本号 zip 容易被误认成当前构建结果，重新打包同一架构时一并清掉。
rm -f "${ARCHIVE}" "${ARCHIVE}.sha256" "${LEGACY_ARCHIVE}" "${LEGACY_ARCHIVE}.sha256" "${PREVIOUS_BRAND_ARCHIVE}" "${PREVIOUS_BRAND_ARCHIVE}.sha256"
