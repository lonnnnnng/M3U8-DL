...
case "${ARCHIVE}" in
  *_desktop_macos_*.zip)
    ...
    require_entry "${entries}" "m3u8dl-go.app/"
    require_entry "${entries}" "m3u8dl-go.app/Contents/MacOS/m3u8dl-go-desktop"
    require_entry "${entries}" "m3u8dl-go.app/Contents/Resources/m3u8dl-go-cli"
