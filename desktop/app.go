...
type Settings struct {
	...
	UseFFmpegMerge          bool     `json:"useFFmpegMerge"`
...
func defaultSettings() Settings {
	return Settings{
		...
		LinkNameSeparator:  defaultLinkNameSeparator,
...
func normalizeSettings(settings Settings) Settings {
	...
	if strings.TrimSpace(settings.LinkNameSeparator) == "" {
		settings.LinkNameSeparator = defaultLinkNameSeparator
	}
	rawOutputFormat := strings.TrimSpace(settings.OutputFormat)
	if rawOutputFormat == "" {
		settings.OutputFormat = defaultOutputFormat
		settings.UseFFmpegMerge = true
	} else {
		settings.OutputFormat = normalizeDesktopOutputFormat(rawOutputFormat)
	}
	if !settings.UseFFmpegMerge && settings.OutputFormat == "mp4" {
		// long: MP4 是封装格式，不能靠二进制直拼得到可用文件；关闭 FFmpeg 合并时保存格式自动落到 TS，避免设置里出现不可执行组合。
		settings.OutputFormat = "ts"
	}
	if strings.TrimSpace(settings.MuxAfterDone) == "" {
		settings.MuxMP4 = settings.UseFFmpegMerge && settings.OutputFormat == "mp4"
		settings.BinaryMerge = !settings.UseFFmpegMerge
	}
	return settings
}
...
func (a *App) CreateTask(req DownloadRequest) (Task, error) {
	requests, err := a.expandDownloadRequests(req)
	if err != nil {
		return Task{}, err
	}
	if len(requests) != 1 {
...
func (a *App) PreflightDownload(req DownloadRequest) PreflightReport {
	...
	for _, item := range requests {
		...
		usesDefaultFFmpegMerge := item.UseFFmpegMerge && strings.TrimSpace(item.MuxAfterDone) == ""
		needsFFmpegForTask := ((usesDefaultFFmpegMerge || item.MuxMP4 || muxAfterDoneNeedsFFmpeg(item.MuxAfterDone)) && !item.SkipDownload && !item.SkipMerge) || item.LivePipeMux
		if needsFFmpegForTask || strings.TrimSpace(item.FFmpegPath) != "" {
			...
			if !checkedFFmpeg[key] {
				...
				if err != nil {
					report.addCheck("ffmpeg", "FFmpeg", "error", "FFmpeg 合并需要可用的 FFmpeg", err.Error())
				} else {
...
func (a *App) StartTask(id string) error {
	...
	if err := a.repairNamedTaskRequestLocked(task); err != nil {
		a.mu.Unlock()
		return err
	}
...
func (a *App) expandDownloadRequests(req DownloadRequest) ([]DownloadRequest, error) {
	a.mu.Lock()
	separator := a.settings.LinkNameSeparator
	a.mu.Unlock()
	if strings.TrimSpace(req.LinkNameSeparator) == "" {
		// long: 桌面端批量地址解析依赖用户配置的保存名分隔符；请求里没带时使用已保存默认值，保证重启后仍按同一格式拆分任务。
		req.LinkNameSeparator = separator
	}
	return expandDownloadRequests(req)
}
...
func (a *App) repairNamedTaskRequestLocked(task *Task) error {
	if task == nil {
		return nil
	}
	req, changed, err := repairNamedDownloadRequest(task.Request, a.settings.LinkNameSeparator)
	if err != nil {
		return err
	}
	if !changed {
	...
	req = applyDesktopMergeMode(req)
	args := buildCLIArgs(req)
	// long: 历史任务可能已经把“名称+分隔符+地址”整行保存进 Request；启动前重建请求和命令，避免重试时继续把名称当成本地文件路径。
	task.Request = req
	task.Title = taskTitle(req)
	task.Args = append([]string(nil), args...)
	task.Command = defaultCLICommandName()
	task.CommandLine = taskCommandLine(defaultCLICommandName(), args)
	return nil
}

func repairNamedDownloadRequest(req DownloadRequest, fallbackSeparator string) (DownloadRequest, bool, error) {
	lines := splitDownloadLines(req.URL)
	if len(lines) != 1 {
		return req, false, nil
	}
	line := lines[0]
	if isDirectDownloadInput(line) {
		if line == req.URL {
			return req, false, nil
		}
		req.URL = line
	...
	for _, separator := range candidateLinkNameSeparators(req.LinkNameSeparator, fallbackSeparator, defaultLinkNameSeparator) {
		saveName, url, ok, err := splitNamedDownloadLine(line, separator)
		if err != nil {
...
func candidateLinkNameSeparators(values ...string) []string {
	seen := map[string]bool{}
	var separators []string
	for _, value := range values {
		separator := strings.TrimSpace(value)
		if separator == "" || seen[separator] {
			continue
		}
		seen[separator] = true
...
func isDirectDownloadInput(input string) bool {
...
func (a *App) applyRequestDefaults(req DownloadRequest) DownloadRequest {
	...
	if strings.TrimSpace(req.LinkNameSeparator) == "" {
		req.LinkNameSeparator = settings.LinkNameSeparator
	}
	if strings.TrimSpace(req.OutputFormat) == "" {
		req.OutputFormat = settings.OutputFormat
		req.UseFFmpegMerge = settings.UseFFmpegMerge
	} else {
		req.OutputFormat = normalizeDesktopOutputFormat(req.OutputFormat)
	}
...
func applyDesktopMergeMode(req DownloadRequest) DownloadRequest {
	...
	if req.OutputFormat == "mp4" {
		req.UseFFmpegMerge = true
	}
	if !req.UseFFmpegMerge {
		// long: 关闭 FFmpeg 合并代表使用原始分片顺序直拼，普通 HLS 点播最终容器只能按 TS 语义处理。
		req.OutputFormat = "ts"
		req.MuxMP4 = false
		req.BinaryMerge = true
		return req
	}
	// long: 桌面端的格式选择最终落到 FFmpeg 混流参数；高级 -M 参数仍由用户显式控制。
	req.MuxMP4 = req.OutputFormat == "mp4"
	req.BinaryMerge = false
	return req
}

func normalizeDesktopOutputFormat(value string) string {
...
func buildCLIArgs(req DownloadRequest) []string {
	...
	if req.UseFFmpegMerge && strings.TrimSpace(req.MuxAfterDone) == "" && !req.SkipDownload && !req.SkipMerge {
		format := normalizeDesktopOutputFormat(req.OutputFormat)
		args = append(args, "-M", "format="+format+":muxer=ffmpeg")
	}
...
func desktopToolCheckSpecs(ffmpegPath string) []toolCheckSpec {
	...
	return []toolCheckSpec{
		...
		{Name: "shaka-packager", Label: "Shaka Packager", Commands: shakaPackagerCandidates(), Args: []string{"--version"}},
	}
}

func shakaPackagerCandidates() []string {
	// long: 外部工具检测失败时会展示最后一个候选的错误，按平台排序可以避免 macOS 用户看到 Windows 包名这种误导信息。
	switch runtime.GOOS {
	case "darwin":
		return []string{"shaka-packager", "packager-linux-x64", "packager-win-x64", "packager-osx-x64"}
	case "windows":
		return []string{"shaka-packager", "packager-linux-x64", "packager-osx-x64", "packager-win-x64"}
	default:
		return []string{"shaka-packager", "packager-osx-x64", "packager-win-x64", "packager-linux-x64"}
	}
}
