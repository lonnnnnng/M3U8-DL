...
func TestDesktopMergeModeDefaultsToFFmpegMP4(t *testing.T) {
	app := newTestApp(t)
	...
	if req.BinaryMerge || !req.UseFFmpegMerge || !req.MuxMP4 || req.OutputFormat != "mp4" {
		t.Fatalf("desktop defaults should use FFmpeg MP4, got %#v", req)
	}
	args := buildCLIArgs(req)
	joined := "\x00" + strings.Join(args, "\x00") + "\x00"
	if !strings.Contains(joined, "\x00-M\x00format=mp4:muxer=ffmpeg\x00") {
		t.Fatalf("default FFmpeg MP4 args missing: %#v", args)
	}
	if strings.Contains(joined, "\x00--binary-merge\x00true\x00") || strings.Contains(joined, "\x00--ffmpeg-binary-path\x00") {
		t.Fatalf("default FFmpeg MP4 should use PATH ffmpeg without binary merge: %#v", args)
	}
}
...
func TestDesktopMergeModeSupportsFFmpegTS(t *testing.T) {
	app := newTestApp(t)
	app.settings.OutputFormat = "ts"
	app.settings.UseFFmpegMerge = true
	req := app.applyRequestDefaults(DownloadRequest{
		URL:            "https://example.com/index.m3u8",
		SaveDir:        filepath.Join(t.TempDir(), "out"),
		AutoSelect:     true,
	...
	if req.BinaryMerge || !req.UseFFmpegMerge || req.MuxMP4 || req.OutputFormat != "ts" {
		t.Fatalf("TS output with FFmpeg should keep FFmpeg merge without MP4 shortcut, got %#v", req)
	}
	joined := "\x00" + strings.Join(buildCLIArgs(req), "\x00") + "\x00"
	if !strings.Contains(joined, "\x00-M\x00format=ts:muxer=ffmpeg\x00") {
		t.Fatalf("FFmpeg TS args missing: %q", joined)
	}
	if strings.Contains(joined, "\x00--binary-merge\x00true\x00") {
		t.Fatalf("FFmpeg TS should not force binary merge: %q", joined)
	}
}

func TestDesktopMergeModeDisablingFFmpegUsesBinaryTS(t *testing.T) {
	app := newTestApp(t)
	app.settings.OutputFormat = "ts"
	app.settings.UseFFmpegMerge = false
	req := app.applyRequestDefaults(DownloadRequest{
		URL:            "https://example.com/index.m3u8",
		SaveDir:        filepath.Join(t.TempDir(), "out"),
		AutoSelect:     true,
	...
	if !req.BinaryMerge || req.UseFFmpegMerge || req.MuxMP4 || req.OutputFormat != "ts" {
		t.Fatalf("disabled FFmpeg merge should use binary TS, got %#v", req)
	}
	joined := "\x00" + strings.Join(buildCLIArgs(req), "\x00") + "\x00"
	if !strings.Contains(joined, "\x00--binary-merge\x00true\x00") {
		t.Fatalf("binary merge args missing: %q", joined)
	}
	if strings.Contains(joined, "\x00-M\x00") {
		t.Fatalf("binary merge should not emit FFmpeg mux args: %q", joined)
	}
}
...
func TestCreateTasksUsesSavedLinkNameSeparator(t *testing.T) {
	app := newTestApp(t)
	...
	if tasks[0].Request.SaveName != "第一集" || tasks[0].Request.URL != "https://example.com/one.m3u8" {
		t.Fatalf("first task should use saved separator, got %#v", tasks[0].Request)
	}
	if tasks[1].Request.SaveName != "第二集" || tasks[1].Request.URL != "https://example.com/two.m3u8" {
		t.Fatalf("second task should use saved separator, got %#v", tasks[1].Request)
	}
	if tasks[0].Request.LinkNameSeparator != "=>" || tasks[1].Request.LinkNameSeparator != "=>" {
		t.Fatalf("created tasks should retain saved separator, got %#v and %#v", tasks[0].Request, tasks[1].Request)
	}
}

func TestRetryTaskRepairsPersistedNamedLineWithSavedSeparator(t *testing.T) {
	app := newTestApp(t)
	fakeCLI := writeFakeCLI(t)
	t.Setenv("M3U8DL_GO_CLI", fakeCLI)
	...
	badRequest := app.applyRequestDefaults(DownloadRequest{
		URL:               "第656集$https://example.com/index.m3u8",
		SaveDir:           out,
		LinkNameSeparator: "|",
		AutoSelect:        true,
		ThreadCount:       4,
	...
	badArgs := buildCLIArgs(badRequest)
	task := &Task{
		ID:          newTaskID(),
		Title:       "index.m3u8",
		Status:      StatusFailed,
		...
		CommandLine: taskCommandLine(defaultCLICommandName(), badArgs),
	}
	app.mu.Lock()
	app.tasks[task.ID] = task
	app.order = []string{task.ID}
	app.mu.Unlock()

	if err := app.RetryTask(task.ID); err != nil {
		t.Fatal(err)
	}
	completed := waitTaskStatus(t, app, task.ID, StatusCompleted, 5*time.Second)
	if completed.Request.URL != "https://example.com/index.m3u8" || completed.Request.SaveName != "第656集" {
		t.Fatalf("retry should repair named URL before launch, got %#v", completed.Request)
	}
	if completed.Title != "第656集" {
		t.Fatalf("repaired task title should use parsed save name, got %q", completed.Title)
	}
	if strings.Contains(strings.Join(completed.Args, "\x00"), "第656集$https://example.com/index.m3u8") {
		t.Fatalf("CLI args should not keep the unparsed named line: %#v", completed.Args)
	}
	if !strings.Contains(strings.Join(completed.Args, "\x00"), "--save-name\x00第656集") {
		t.Fatalf("CLI args should include parsed save name: %#v", completed.Args)
	}
	if _, err := os.Stat(filepath.Join(out, "第656集.mp4")); err != nil {
		t.Fatalf("fake CLI should write output with parsed save name: %v", err)
	}
}
...
func TestStopTaskTerminatesChildProcessGroup(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("process group stop is covered by Unix-like runners")
	}
	app := newTestApp(t)
	fakeCLI := writeSlowContinuingFakeCLI(t)
	t.Setenv("M3U8DL_GO_CLI", fakeCLI)

	task, err := app.StartDownload(DownloadRequest{
		URL:            "https://example.com/slow-child.m3u8",
		SaveDir:        filepath.Join(t.TempDir(), "out"),
		SaveName:       "slow-child",
		AutoSelect:     true,
		ThreadCount:    4,
	...
	waitUntilTaskRunning(t, app, task.ID)
	if err := app.StopTask(task.ID); err != nil {
		t.Fatal(err)
	}
	stopped := waitTaskStatus(t, app, task.ID, StatusStopped, 3*time.Second)
	if strings.Contains(strings.Join(stopped.Logs, "\n"), "不应在停止后继续输出") {
		t.Fatalf("child process should be terminated with the task, logs=%#v", stopped.Logs)
	}
}
...
func writeSlowContinuingFakeCLI(t *testing.T) string {
	t.Helper()
	if runtime.GOOS == "windows" {
		t.Skip("fake shell CLI test is only used on Unix-like CI runners")
	}
