...
- 暂未完全追平：SAMPLE-AES 媒体级原生解密、CENC KID 自动读取已支持结构化 MP4 info、`schm`、`tenc`、Widevine PSSH data、PSSH v1 KID 列表、PlayReady PSSH XML 文本/`VALUE` 属性 KID 解析、`tenc default_KID` 全 0 时按上游回退 Widevine PSSH 真实 KID 和 MultiDRM track/label `1` 参数，以及 shaka 缺 key 错误探测 `key_id`，但仍缺更复杂 DRM 样本的系统验证；实时解密仍是基础外部工具流程而非完整上游等价实现、完整 ANSI 进度 UI、多语言资源已覆盖核心运行输出但尚未复刻完整 `ResString` 资源表、直播 producer/consumer 主循环已覆盖非字幕实时合并和 macOS/Linux pipe mux 批次写入，Windows 命名管道已实现并通过交叉编译，仍未达到上游完整多轨状态机。
