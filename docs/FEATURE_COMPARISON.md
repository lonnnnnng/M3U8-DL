...
- 过滤条件包括 ID、语言、名称、编码、分辨率、帧率、声道、HDR/DV、URL、分片数、播放列表时长、码率范围。
...
| HTTP Header | 默认 UA、自定义 Header、多 Header | 已支持；跳转后保留 Header/Range | 已追平 |
...
| 选择/丢弃过滤 | `-sv/-sa/-ss`、`-dv/-da/-ds` | 已支持主要过滤条件、`bestN/worstN/all`、先 drop 后 keep | 已追平 |
...
| 多语言资源 | zh-CN/zh-TW/en-US 完整资源 | Go 版做了参数校验和部分输出，未复刻完整资源系统 | 未追平 |
