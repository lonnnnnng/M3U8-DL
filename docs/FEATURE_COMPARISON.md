...
| MP4 实时解密 | `--mp4-real-time-decryption` | 已覆盖 init+fragment 基础实时外部解密；init 会先保留原始盒读取 KID，本地读不到且使用 shaka 时会从缺 key 错误探测 `key_id`，再解密 init/媒体分片，并在实时解密开启时跳过最终整文件二次解密 | 部分追平 |
...
| ANSI 进度 UI | Spectre Console 动态进度列 | Go 版有基础日志/输出，没有完整 ANSI 进度 UI | 未追平 |
| 多语言资源 | zh-CN/zh-TW/en-US 完整资源 | Go 版已接入核心运行输出、自动派生选项提示、更新检查提示和下载进度文本，完整 `ResString` 资源表仍未复刻 | 部分追平 |
| 更新检查 | GitHub latest release | 已支持，可 `--disable-update-check` 禁用 | 已追平 |
| 日志文件 | 日志路径、等级、禁用 | 已支持 `--log-file-path`、`--log-level`、`--no-log` | 已追平 |
...
2. MP4 实时解密已补齐 init 先读 KID、shaka 缺 key 探测 KID、合并后不二次整文件解密的路径，但还没有达到上游直播状态机里所有边缘分支的等价程度。
