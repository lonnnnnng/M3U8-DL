...
func TestParseArgsMuxAfterDoneStrictValidation(t *testing.T) {
	cases := []struct {
		...
		{name: "skip_sub", mux: "format=mp4:skip_sub=yes", want: "skip_sub=false not valid"},
		{name: "skip_sub with keep", mux: "format=mp4:keep=true:skip_sub=yes", want: "skip_sub=true not valid"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestLiveSegmentTempPathUsesProgramDateTimeLikeUpstream(t *testing.T) {
	...
	if got != filepath.Join("/tmp/live", "1781784000000.ts") {
...
func TestLiveSegmentKeyUsesDateTime(t *testing.T) {
	...
	if liveSegmentKey(Segment{DateTime: &now, URL: "x"}) != fmt.Sprintf("%d", now.UnixMilli()) {
...
func TestCurrentVersionTag(t *testing.T) {
	if currentVersionTag("m3u8dl-go 0.3.0") != "v0.3.0" {
...
func TestLiveRealtimeSegmentNamesUseCurrentBatchProgramDateTime(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(state.tmpDir, "1781784000000.ts")); err != nil {
